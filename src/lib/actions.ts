...
export async function createTask(projectId: number, formData: FormData) {
  ...
  await query(`
    INSERT INTO "project_task" ("project_id", "name", "quantity", "unit_price", "date_start", "date_end", "status", "consumed_quantity", "daily_consumption")
...
export async function deleteTask(taskId: number, projectId: number) {
    await query(`DELETE FROM "project_task" WHERE id = $1`, [taskId]);
    revalidatePath(`/projects/${projectId}`);
    revalidatePath(`/dashboard`);
    return { success: true };
}
...
export async function deleteMultipleTasks(taskIds: number[], projectId: number | undefined) {
    ...
    await query(`DELETE FROM "project_task" WHERE id IN (${placeholders})`, taskIds);
    revalidatePath(`/projects/${projectId}`);
    revalidatePath(`/dashboard`);
    return { success: true };
}
...
export async function updateTaskConsumption(taskId: number, date: string, consumedQuantity: number) {
    const result = await query<RawTask>(`SELECT * FROM "project_task" WHERE id = $1`, [taskId]);
    ...
    const dailyConsumption = (task.daily_consumption || []).map(dc => ({
      ...dc,
...
export async function validateTask(formData: FormData) {
    ...
    await query(`
      INSERT INTO "project_task_validation" ("task_id", "date", "image_url", "location")
      VALUES ($1, $2, $3, $4)
...
export async function deleteValidation(validationId: number, projectId: number) {
    await query(`DELETE FROM "project_task_validation" WHERE id = $1`, [validationId]);
    revalidatePath(`/projects/${projectId}`);
    return { success: true };
}
